...
classdef MLP1
    ...
    methods
        function clf = MLP1(x_train,y_train,hiddenLayers,nIter,learningRate)            
                ...
                for j = 1:clf.nSamples
                    ...
                    for k = 1:numel(clf.hiddenLayers)
                        ...
                        for l = 1:clf.hiddenLayers(k)
                            val{k}(l) = clf.RELU([1.0 prev] * clf.W{k}(l,:)');
                        end
                        prev = val{k}(:)';
                    end
                    val{numel(clf.hiddenLayers)+1} = zeros(numel(clf.Classes),1);
                    for k = 1:numel(clf.Classes)
                        val{numel(clf.hiddenLayers)+1}(k) = [1.0 prev] * clf.W{numel(clf.hiddenLayers)+1}(k,:)';
                    end
                    for k = 1:numel(clf.Classes)
                        val{numel(clf.hiddenLayers)+1}(k) = clf.softmax(val{numel(clf.hiddenLayers)+1}, val{numel(clf.hiddenLayers)+1}(k));
                    end
                    ...
                    for l = 1:numel(clf.Classes)
                        ...
                        delta{numel(clf.hiddenLayers)+1}(l) = (expected - output) * output * (1 - output);
                    end
        ...
        function val = softmax(clf, array, numerator)
            ...
            for i = 1:size(x_test,1)
                ...
                for j = 1:numel(clf.hiddenLayers)
                    ...
                    for k = 1:clf.hiddenLayers(j)
                        val{j}(k) = clf.RELU([1.0 prev] * clf.W{j}(k,:)');
                    end
                    prev = val{j}(:)';
                end
                val{numel(clf.hiddenLayers)+1} = zeros(numel(clf.Classes),1);
